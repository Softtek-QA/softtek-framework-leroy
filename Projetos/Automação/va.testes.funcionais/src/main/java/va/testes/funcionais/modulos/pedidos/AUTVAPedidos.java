...
public class AUTVAPedidos {
	...
	public void test() {
		...
		String materialCodigoProduto = "88282446";
		Integer materialQuantidade = 5;		
		...
		AUTVAUtilidades.enviarDadosElementWeb(AUTVAModuloCadastroClientesPF.docDriver.findElementById("search"), (long)0.4, materialCodigoProduto);
		...
		for(org.openqa.selenium.WebElement item : itensEncontrado) {
			item.clear();
			item.sendKeys(materialQuantidade.toString());
			item.sendKeys("\t");
		}
		...
		java.util.List<org.openqa.selenium.WebElement> botaoCarrinho = AUTVAUtilidades.procurarElementWebHTML(AUTVAModuloCadastroClientesPF.docDriver.getClass().getName(), AUTVAModuloCadastroClientesPF.docDriver, (long)0.3, "button", "(?i:\\<(span).{0,}\\>\\W{0,}.{0,}carrinho\\W{0,}.{0,})");
